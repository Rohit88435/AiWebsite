...
function WebsiteEditor() {
  ...
  useEffect(() => {
    if (!ifreameRef.current || !code) {
      return;
    }
    const blob = new Blob([code], { type: "text/html" });
    const url = URL.createObjectURL(blob);
    ifreameRef.current.src = url;
      ...
      <AnimatePresence>
        {showChat && (
          <motion.div
            ...
            animate={{ y: 0 }}
            exit={{ y: "100%" }}
            className="fixed inset-0  z-999 bg-[#1e1e1e] flex flex-col"
          >
            <Header />
            <>
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
  ...
  function Header() {
    return (
      <div className="h-14 px-4 flex items-center justify-between border-b border-white/10">
        ...
        <button className="lg:hidden" onClick={() => setShowChat(false)}>
          <X size={18} />
        </button>
      </div>
    );
